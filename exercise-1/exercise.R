...
ggplot(data = diamonds.sample) +
  geom_line(mapping = aes(x = carat, y = price, color = carat), size = 1)

# That's kind of messy. Try using `smooth` geometry instead.
ggplot(data = diamonds.sample) +
  geom_smooth(mapping = aes(x = carat, y = price, color = carat), size = 1)

# Draw a plot with bar geometry (a bar chart), mapping the diamond's `cut` to the x-axis
ggplot(data = diamonds.sample) +
  geom_bar(mapping = aes(x = cut, color = carat), size = 20)
...
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = carat, y = price), size = 3, color = "blue", alpha = 0.1) +
  geom_smooth(mapping = aes(x = carat, y = price), size = 2, color = "pink", se = FALSE)
